...
class döviz(commands.Cog):
    ...
    async def döviz(self,ctx,int=1):
        ...
        embed=discord.Embed(title="Güncel Döviz Kurları ve Borsalar",inline=False)
        embed.add_field(name="GRAM ALTIN",value=":chart_with_downwards_trend: {} ₺".format(altın),inline=False)
        embed.add_field(name="ABD DOLARI          ",value=":chart_with_downwards_trend: {} ₺".format(dolar),inline=False)
        embed.add_field(name="EURO",value=":chart_with_downwards_trend: {} ₺".format(euro),inline=False)
        embed.add_field(name="BİTCOİN",value=":chart_with_downwards_trend: {}".format(bitcoin),inline=False)
